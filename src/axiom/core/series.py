...
class OHLCVSeries:
    ...
    def opens(self) -> np.ndarray:
        return np.asarray(self.df["open"].to_numpy())

    @property
    def highs(self) -> np.ndarray:
        return np.asarray(self.df["high"].to_numpy())

    @property
    def lows(self) -> np.ndarray:
        return np.asarray(self.df["low"].to_numpy())

    @property
    def closes(self) -> np.ndarray:
        return np.asarray(self.df["close"].to_numpy())

    @property
    def volumes(self) -> np.ndarray:
        return np.asarray(self.df["volume"].to_numpy())
    ...
    def atr(self, period: int = 14) -> np.ndarray:
        ...
        if n <= period:
            return np.asarray(np.cumsum(tr) / np.arange(1, n + 1))
